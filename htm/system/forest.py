...
class Forest:
    def __init__(self, cs = None, cp = None, numDenPFor=None, numSynPDen=None, threshPercent=None):
        ...
        self.numSpD = numpy.uint32(numSynPDen) # number of synapses per dendrite 
        ...
        self.dThresh = numpy.uint32(self.numSpD * threshPercent -1) # dendrite activation threshold (how many active synapses needed to activate dendrite)
        if self.dThresh == numpy.uint32(0):
            self.dThresh = numpy.uint32(1)
        ...
        pyopencl.enqueue_fill_buffer(self.cs.getQueue(), self.bufferSAddrs, numpy.uint32(numpy.iinfo(numpy.uint32()).max), 0, self.numbytesSAddrs).wait()
        pyopencl.enqueue_fill_buffer(self.cs.getQueue(), self.bufferSPerms, numpy.uint8(0), 0, self.numbytesSPerms).wait()
